% -------------------------------------------------------------------------
% Script:       Rdosl_Simulation_Validation
...
%
% Description:
%   Validates the analytical approximation of the dipoalr specific rate Rdosl
%   against the exact numerical solution using White Matter 
%   tissue parameters.
%
% Outputs:
%   - Fig 1: Rdosl sensitivity to Frequency Offset (dw).
%   - Fig 2: Rdosl sensitivity to RF Amplitude (w1).
...
Rdosl_exact_dw  = zeros(n_t1d_plot, n_dw);
Rdosl_approx_dw = zeros(n_t1d_plot, n_dw);
...
for k = 1:n_t1d_plot
    ...
    for i_dw = 1:n_dw
        ...
        Rdosl_exact_dw(k, i_dw)  = cal_Rdosl_exact(R1a, R2a, R1b, MPF, R, T2b, curr_T1d, dw,  w1_fixed);
        Rdosl_approx_dw(k, i_dw) = cal_Rdosl_approx(R, T2b, curr_T1d, dw,  w1_fixed, B1, B0);
    end
end
...
Rdosl_exact_w1  = zeros(n_t1d_plot, n_w1);
Rdosl_approx_w1 = zeros(n_t1d_plot, n_w1);
...
for k = 1:n_t1d_plot
    ...
    for i_w1 = 1:n_w1
        w1 = w1_all(i_w1) * 2 * pi;
        Rdosl_exact_w1(k, i_w1)  = cal_Rdosl_exact(R1a, R2a, R1b, MPF, R, T2b, curr_T1d, dw_fixed,  w1);
        Rdosl_approx_w1(k, i_w1) = cal_Rdosl_approx(R, T2b, curr_T1d, dw_fixed,  w1, B1, B0);
    end
end
...
Rdosl_approx_map = zeros(n_dw, n_w1);
Rdosl_exact_map  = zeros(n_dw, n_w1);
...
for i_dw = 1:n_dw
    ...
    for i_w1 = 1:n_w1
        w1 = w1_all(i_w1) * 2 * pi;
        Rdosl_exact_map(i_dw, i_w1)  = cal_Rdosl_exact(R1a, R2a, R1b, MPF, R, T2b,T1d_map, dw,  w1);
        Rdosl_approx_map(i_dw, i_w1) = cal_Rdosl_approx(R, T2b,T1d_map, dw,  w1, B1, B0);
    end
end
...
for k = 1:n_t1d_plot
    ...
    p_handles(k) = plot(dw_all/1000, Rdosl_exact_dw(k,:), '-', ...
    ...
    plot(dw_all(idx_sample)/1000, Rdosl_approx_dw(k,idx_sample), 'o', ...
...
xlabel('Frequency Offset $\Delta\omega/2\pi$ (kHz)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('$R_{dosl}$ (Hz)', 'Interpreter', 'latex', 'FontSize', 14);
title({['$R_{dosl}$ vs. Frequency Offset'], ...
...
for k = 1:n_t1d_plot
    % Exact Solution
    p_handles_2(k) = plot(w1_all, Rdosl_exact_w1(k,:), '-', ...
        'Color', colors(k,:), 'LineWidth', 2);
    
    % Approx Solution (Subsampled)
    idx_sample = 1:3:length(w1_all);
    plot(w1_all(idx_sample), Rdosl_approx_w1(k,idx_sample), 'o', ...
        'Color', colors(k,:), 'MarkerFaceColor', 'w', 'MarkerSize', 6);
end

% Labels and Title
% [FIXED]: Removed erroneous %d and added $ signs
xlabel('RF Amplitude $\omega_1/2\pi$ (Hz)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('$R_{dosl}$ (Hz)', 'Interpreter', 'latex', 'FontSize', 14);
title({['$R_{dosl}$ vs. RF Amplitude'], ...
...
% -------------------------------------------------------------------------
RelError = abs(Rdosl_approx_map - Rdosl_exact_map) ./ Rdosl_exact_map * 100;
...
xlabel('RF Amplitude $\omega_1/2\pi$ (Hz)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Frequency Offset $\Delta\omega/2\pi$ (kHz)', 'Interpreter', 'latex', 'FontSize', 14);

title(['Relative Error Map ($T_{1D} = ' num2str(T1d_map*1000) '\,\mathrm{ms}$)'], 'Interpreter', 'latex', 'FontSize', 14);
